...
def Add():
    ...
    mysqldb=mysql.connector.connect(host="localhost",user="root",password="214531",database="new")
    mycursor=mysqldb.cursor()

    try:
       sql = "INSERT INTO  registation (empname,mobile,salary) VALUES (%s, %s, %s)"
...
def update():
    ...
    try:
       sql = "Update  registation set mobile= %s,salary= %s where id= %s"
       val = (studname,coursename,feee)
       mycursor.execute(sql, val)
       mysqldb.commit()
       lastid = mycursor.lastrowid
...
def delete():
    ...
    try:
       sql = "delete from registation where name = %s"
       val = (studid,)
       mycursor.execute(sql, val)
       mysqldb.commit()
       lastid = mycursor.lastrowid
